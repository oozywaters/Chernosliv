...
@interface TableViewBindingHelper () <UITableViewDataSource, UITableViewDelegate, ObservableMutableArrayDelegate>
...
- (instancetype)initWithTableView:(UITableView *)tableView
    ...
    if (self = [super init]) {
        ...
        [source subscribeNext:^(id x) {
            if ([x isKindOfClass:[ObservableMutableArray class]]) {
                ((ObservableMutableArray *)x).delegate = self;
            }
            if (self->_data != nil && [self->_data isKindOfClass:[ObservableMutableArray class]]) {
                ((ObservableMutableArray *)self->_data).delegate = nil;
            }
            self->_data = x;
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        if ([_data isKindOfClass:[ObservableMutableArray class]]) {
            [((ObservableMutableArray *)_data) removeObjectAtIndex:indexPath.row];
        } else {
            NSLog(@"The array bound to the table view must be a ObservableMutableArray");
        }
    }
}
...
#pragma mark - ObservableMutableArrayDelegate methods
...
- (void)array:(ObservableMutableArray *)array didAddObjectsFromArray:(NSArray *)otherArray {
    NSUInteger currentIndex = array.count - otherArray.count;
